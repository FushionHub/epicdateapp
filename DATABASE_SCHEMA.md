...
CREATE POLICY "Allow public read access to non-sold listings" ON public.listings
  FOR SELECT USING (is_sold = false);

CREATE POLICY "Allow users to manage their own listings" ON public.listings
  FOR ALL USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);
```

### For Listing Images
1.  Go to the "Storage" section in your Supabase dashboard and create a **public** bucket named `listing-images`.

---

## Part 12: Marketplace Functions

```sql
-- Function to create a private chat match between a buyer and a seller
CREATE OR REPLACE FUNCTION public.create_conversation_for_listing(p_seller_id UUID)
RETURNS BIGINT
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  FROM public.matches
  WHERE (user1_id = v_buyer_id AND user2_id = p_seller_id)
     OR (user1_id = p_seller_id AND user2_id = v_buyer_id)
  LIMIT 1;

  -- If no match exists, create one
  IF v_match_id IS NULL THEN
    INSERT INTO public.matches (user1_id, user2_id)
    VALUES (v_buyer_id, p_seller_id)
